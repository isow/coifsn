...
use Database\TDBM\Filters\OrderBySQLString;

use Database\TDBM\Filters\SqlStringFilter;
use Database\TDBM\Filters\AndFilter;
use Database\DBConnection\CachedConnection;
use Database\TDBM\Filters\FilterInterface;
use Database\DBConnection\MySqlConnection;
use Database\DBConnection\ConnectionInterface;
...
class TDBMService {
	...
	public function setConnection(ConnectionInterface $connection) {
		if ($this->cacheService != null && !($connection instanceof DB_CachedConnection)) {
			$cachedConnection = new Database\DBConnection\DB_CachedConnection();
	...
	public function getObject($table_name, $filters, $className = null, $lazy_loading = false) {
		...
		if ($className == null) {
			...
			if (!is_subclass_of($className, "Database\\TDBM\\TDBMObject")) {
...
// 			}
	...
	public function buildFilterFromFilterBag($filter_bag) {
		...
		elseif (is_a($filter_bag, 'Database\\TDBM\\TDBMObjectArray')) {
			$filter_bag = array($filter_bag);
		}
		...
		foreach ($filter_bag as $thing) {
			if (is_a($thing,'Database\\TDBM\\Filters\\FilterInterface')) {
				$filter_bag2[] = $thing;
			} elseif (is_a($thing,'Database\\TDBM\\TDBMObject')) {
			...
			} elseif (is_a($thing,'Database\\TDBM\\TDBMObjectArray') && count($thing)>0) {
			...
			} elseif (!is_a($thing,'Database\\TDBM\\TDBMObjectArray') && $thing!==null) {
	...
	public function buildOrderArrayFromOrderBag($orderby_bag) {
		...
		foreach ($orderby_bag as $thing) {
			if (is_a($thing,'Database\\TDBM\\Filters\\OrderBySQLString')) {
				$orderby_bag2[] = $thing;
			} elseif (is_a($thing,'Database\\TDBM\\Filters\\OrderByColumn')) {
				$orderby_bag2[] = $thing;
			} elseif (is_string($thing)) {
